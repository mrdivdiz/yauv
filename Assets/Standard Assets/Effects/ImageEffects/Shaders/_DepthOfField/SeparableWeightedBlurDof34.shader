Shader "Hidden/SeparableWeightedBlurDof34" {
	...
	fixed4 fragBlurUnweighted (v2f i) : SV_Target {
		fixed4 blurredColor = fixed4 (0,0,0,0);

		fixed4 sampleA = tex2D(_MainTex, i.uv.xy);
		fixed4 sampleB = tex2D(_MainTex, i.uv01.xy);
		fixed4 sampleC = tex2D(_MainTex, i.uv01.zw);
		fixed4 sampleD = tex2D(_MainTex, i.uv23.xy);
		fixed4 sampleE = tex2D(_MainTex, i.uv23.zw);
	...
	fixed4 fragBlurWeighted (v2f i) : SV_Target {
		fixed4 blurredColor = fixed4 (0,0,0,0);
		...
		fixed sum = sampleA.a + dot (fixed4 (1.25, 1.25, 1.5, 1.5), fixed4 (sampleB.a,sampleC.a,sampleD.a,sampleE.a));
	...
	fixed4 fragBlurDark (v2f i) : SV_Target {
		fixed4 blurredColor = fixed4 (0,0,0,0);
		...
		half sum = sampleA.a + dot (fixed4 (0.75, 0.75, 0.5, 0.5), fixed4 (sampleB.a,sampleC.a,sampleD.a,sampleE.a));
	...
	fixed4 fragMixMediumAndLowTap (v2fSingle i) : SV_Target 
	{
	 	fixed4 tapMedium = tex2D (_TapMedium, UnityStereoScreenSpaceUVAdjust(i.uv.xy, _TapMedium_ST));
		fixed4 tapLow = tex2D (_TapLow, UnityStereoScreenSpaceUVAdjust(i.uv.xy, _TapLow_ST));
