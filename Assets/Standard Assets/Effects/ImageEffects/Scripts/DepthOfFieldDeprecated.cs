...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class DepthOfFieldDeprecated : PostEffectsBase
    {
		...
		void Update(){
			...
			if(Physics.Raycast(transform.position, transform.forward, out rch, 80, 1))
		 {
			
                    //dist = Mathf.Lerp(dist, Vector3.Distance(transform.position, rch.point) / 2, Time.deltaTime * 3);
                    focalPoint = Mathf.Lerp(focalPoint, Vector3.Distance(transform.position, rch.point) / 2, Time.deltaTime * 2);
					//focus.farFalloff = Mathf.Lerp(focus.farFalloff, Vector3.Distance(transform.position, hit.point) * (20 * Mathf.Clamp(Vector3.Distance(transform.position, hit.point) / 5, 0, 1)), Time.deltaTime * 15);
         }
		 /*if(dist > 2.3f){
		 if(dist > 3.0f){
         focalDistance01 = 3.0f;
		 }
		 if(dist <= 3.0f){
			 focalDistance01 = dist;
		 }
		 }if(dist <= 2.3f){
			 focalDistance01 = 2.3f;
		 }*/
